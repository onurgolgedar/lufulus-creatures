function SL_sprite_set_ambient(argument0) {
	...
	for (i=0; i<nbr; i+=1)
	{
	    surface_s[i] = create_surface(sl_w,sl_h);
	    surface_set_target(surface_s[i]);
	    draw_clear(c_black);
	    surface_reset_target();

	    surface_a[i] = create_surface(sl_w,sl_h);
	...
	for (i=0; i<nbr; i+=1)
	{
	    ...
	    free_surface(surface_s[i]);
	    free_surface(surface_a[i]);
	}
