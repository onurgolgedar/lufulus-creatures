...
#region ROAD QUEST COMPLETE
if (is_quest_incomplete(objQuestRoad_scout2)) {
	quest_progress(objQuestRoad_scout2, 1)
		
	dialogue_progress(1, 1, id)
	
	fm_delete(16)
	add_quest(objQuest_meetSenior_prep)
	add_quest(objQuest_broccoli)
	
	exit
}
#endregion
#region LORD LOVES QUEST COMPLETE
else if (instance_exists(efboxPowerQ)) {
	quest_progress(objQuest_lordLoves, 1)
	
	add_title(title_strong, eng() ? "The lord immediately realized your muscular arms." : "Lord kollarınızdaki kuvveti anında farketti.")
	
	instance_destroy(efboxPowerQ)
...
if (ds_list_size(finishedQuests) > 0) {
	...
	if (quest.object_index == objQuest_rebelWar) {
		dialogue_progress(3, 1, id)
		add_quest(objQuest_lordBusy_prep)
	}
	#endregion
...
, string_return(eng() ? "[ASK THE LETTER]" : "[MEKTUBU SOR]", quest_exists(objQuest_bloodMineral_2))
