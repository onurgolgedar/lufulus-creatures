...
var limitZoom_h = view_get_hport(0)*(0.785-0.3*IS_MOBILE)/(view_get_hport(0)/global.portHeight_real)
if (camera_get_view_height(global.camera)*0.95 > limitZoom_h or global.zoomMode) {	
	camera_set_view_size(global.camera,
	floor(camera_get_view_width(global.camera)*0.95),
	floor(camera_get_view_height(global.camera)*0.95))
