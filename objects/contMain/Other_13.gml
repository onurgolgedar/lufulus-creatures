...
if ((room == roomMenu or room == roomCinematic) and instance_number(parWindow) == 0) {
	if (room == roomCinematic)
		window = show_dialoguebox(15, 40, "", "[c="+string(c_white)+"]"+(eng() ? "Paused" : "Duraklatıldı")+"[/c]", id, 1, eng() ? "Main Menu" : "Ana Menü", eng() ? "Exit Game" : "Oyunu Kapat", string_return(window_get_fullscreen() ? (eng() ? "Windowed Mode" : "Pencere Modu") : (eng() ? "Fullscreen Mode" : "Tam Ekran Modu"), os_type == os_windows), eng() ? "Continue" : "Devam")
	else
