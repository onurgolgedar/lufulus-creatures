...
draw_set_alpha(mainAlpha)
	...
	draw_set_color(c_ltgray) draw_set_alpha(mainAlpha/2)
		draw_rectangle(x+5, y+4+70+10*IS_MOBILE, m_end_x-4,
		m_end_y-4-43, 0)
	draw_set_color(c_black) draw_set_alpha(mainAlpha/3)
		if (!isActive or page == 3)
			draw_rectangle(x+5, y+4+73+52, x+horizontalBoxCount*(boxEdge+boxBetween)-boxBetween+offset_x*2-4-cut_right, y+217, 0)
    ...
    draw_set_font(fontMessageTitle) draw_set_color(c_white)
		...
		if (page == 1)
			...
			title = ltext_passives
		else if (page == 3)
			title = ltext_supershock
        draw_text(x+offset_x, y+offset_x+31+IS_MOBILE*12/*special*/, title)
    draw_set_font(global.mainFont) draw_set_color(c_black)
	
	if (!isActive or page == 3) {
		draw_set_font(fontMessageTitle)
		if (!isActive) draw_set_color(c_red)
			draw_text(x+offset_x+5/*(2*x+horizontalBoxCount*(boxEdge+boxBetween)-boxBetween+offset_x*2-4-cut_right+5)/2*/, y+offset_x+125+IS_MOBILE*4/*special*/, page == 3 ? ltext_supershock_upgrade : ltext_skillswill)
		draw_set_default()
	}
    ...
    draw_set_font(fontPoints) draw_set_color(c_white) draw_set_valign(fa_bottom)
		...
		"[c="+string((global.skillPoints == 0 or !isActive) ? c_white : c_ltgreen)+"]"+ltext_availablepoints+(isActive ? string(points) : "???")+"[/c]")
	...
	if (isActive) {
	    // Page 3 button
	    pageButton3Width = 130
		pageButton3Height = 30+IS_MOBILE*12
	    pageButton_x[3] = exitButton_x-offset_x-pageButton3Width
	    ...
	    draw_set_center()
	        draw_text(pageButton_x[3]+pageButton3Width/2, pageButton_y[3]+pageButton3Height/2, ltext_supershock)
	    draw_set_default()
		
	    // Page 2 button
	    pageButton2Width = 150
		pageButton2Height = 30+IS_MOBILE*12
	    pageButton_x[2] = pageButton_x[3]-offset_x-pageButton2Width
	    ...
	    draw_set_center()
	        draw_text(pageButton_x[2]+pageButton2Width/2, pageButton_y[2]+pageButton2Height/2, ltext_passives)
	    draw_set_default()
    
	    // Page 1 button
	    pageButton1Width = 150
		pageButton1Height = 30+IS_MOBILE*12
	    pageButton_x[1] = pageButton_x[2]-offset_x-pageButton1Width
	    ...
	    draw_set_center()
	        draw_text(pageButton_x[1]+pageButton1Width/2, pageButton_y[1]+pageButton1Height/2, ltext_actives)
	    draw_set_default()
	}
